		...
		if ( maps\mp\zombies\_zm_pers_upgrades::is_pers_system_active() )
		{
			if ( !( isdefined( self.pers_upgrades_awarded["board"] ) && self.pers_upgrades_awarded["board"] ) )
			{
				if ( level.round_number >= level.pers_boarding_round_start )
				{
					...
					if ( self getpersstat( "pers_boarding" ) >= level.pers_boarding_number_of_boards_required )
					{
...
pers_check_for_pers_headshot( time_of_death, zombie )
{
	if ( maps\mp\zombies\_zm_pers_upgrades::is_pers_system_active() )
	{
		if ( self getpersstat("last_headshot_kill_time") == time_of_death )
		...
		if ( self getpersstat( "zombies_multikilled" ) == 2 )
		{
			if ( isdefined( zombie ) )
...
cash_back_player_drinks_perk()
{
	if ( maps\mp\zombies\_zm_pers_upgrades::is_pers_system_active() )
	{
		if ( isdefined( level.pers_upgrade_cash_back ) && level.pers_upgrade_cash_back )
		{
			...
			else if ( self getpersstat( "pers_cash_back_bought" ) < level.pers_cash_back_num_perks_required )
...
pers_nube_unlock_watcher()
{
	...
	if ( self getpersstat( "pers_max_round_reached" ) >= level.pers_nube_lose_round )
		return;

	num_melee_kills = self getpersstat( "melee_kills" );
	num_headshot_kills = self getpersstat( "headshots" );
	num_boards = self getpersstat( "boards" );
	...
	while ( true )
	{
		...
		if ( num_melee_kills == self getpersstat( "melee_kills" ) && num_headshot_kills == self getpersstat( "headshots" ) && num_boards == self getpersstat( "boards" ) )
